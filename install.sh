...
def dim(msg):  print(f"{C['D']}{msg}{C['X']}")
...
def manifest_path(mount):
    return os.path.join(mount, ".manifest.enc")
...
def random_basename():
    return f"{secrets.token_hex(10)}.sparsebundle"

def resolve_path(input_path):
    """Sanitize drag-and-drop paths: strip backslashes, expand ~, detect .app"""
    p = input_path.replace('\\', '')
    p = os.path.expanduser(p)
    p = p.rstrip('/')
    if os.path.exists(p):
        return p
    if os.path.exists(p + '.app'):
        return p + '.app'
    return p

def add_one(src, mount, passphrase, manifest):
    """Add a single file to the open vault. Returns updated manifest."""
    ...
    if os.path.exists(dst):
        ...
        return manifest
    shutil.move(str(Path(src).resolve()), dst)   # instant rename, not copy
    manifest[key] = h
...
def remove_one(orig_key, mount, passphrase, manifest):
    """Remove a single file from the open vault. Returns updated manifest."""
...
def cmd_open(args):
    if args.vault:
        ...
        if not vault_path:
            die("No store found. Run '" + BIN_NAME + " init' first.")

    vault = Path(vault_path)
    if not vault.exists():
        die(f"Store not found: {vault}")

    if read_state():
        die("A store is already open. Run '" + BIN_NAME + " close' first.")
        ...
        try:
            while True:
                ...
                try:
                    idx = int(choice) - 1
                    if 0 <= idx < len(items):
                        orig_key = items[idx][0]
                        manifest = remove_one(orig_key, mount, passphrase, manifest)
                        items = sorted(manifest.items())
                        if not items:
...
def cmd_restore_inner(mount, passphrase, manifest):
    """Restore all files, return empty manifest."""
...
def cmd_restore(args):
    ...
    manifest = read_manifest(mount, passphrase)
    if not manifest:
        info("Store is already empty.")
        return
    cmd_restore_inner(mount, passphrase, manifest)
    ok("All files restored. Store is now empty.")

def cmd_show(args):
    """Restore all tracked files to origin, keep tracking."""
    st = require_open()
    mount, passphrase = st["mount"], st["passphrase"]
    manifest = read_manifest(mount, passphrase)
...
def cmd_hide(args):
    """Hide all tracked files back into the store. Instant rename."""
    st = require_open()
    mount, passphrase = st["mount"], st["passphrase"]
    manifest = read_manifest(mount, passphrase)
    if not manifest: info("Nothing tracked."); return
...
def main():
    ...
    cmds = {
        ...
        "show": cmd_show, "hide": cmd_hide,
        "restore": cmd_restore, "status": cmd_status, "uninstall": cmd_uninstall,
    }
